# AS3935 Lightning Detector Node — Engineering Notebook

*Per-strike lightning detection for Home Assistant / ESPHome*
*Compiled July 2026; last updated 2026-09-22*

> **This is the project's working history**, kept in full: what was tried, what failed, what
> was retracted, and why. For the finished thing — what it is, what it is made of, and how to
> build one — read the **[project README](../README.md)**. Everything here is the record
> behind it, and every `§` reference in the repository points into this file.

---

## Project status — read this first

**It works, and it has now seen real lightning.** Across four days in the bedroom closet the node
detected three thunderstorms, reporting ranged distances from **5 to 37 km** with widely varying
energies, correlating storm-for-storm with the WS90's independent count (**§11.9**). The
out-of-range `63` code that made every earlier "strike" suspect did not appear once. That closes
the question the project had been unable to answer since July: **the detector detects lightning.**

**The false-lightning rate is effectively zero.** Outside the storm windows, four days of closet
operation produced no lightning classifications at all — against the breadboard's 3–8 per minute
(§11.2). The instrument is quiet enough that the events it does report can be believed.

**Installed in its final location on 2026-09-22 at 12:20 — the garage attic (§11.10).** Both
boxes moved together from the closet. WiFi −49 to −55 dBm, ESP32 internal temperature settled at
~50 °C on a September afternoon, consistent with §10's ~52 °C attic figure. **No liveness check
has been run since the move**, so the attic's first quiet hours mean nothing yet — §11.7's rule
applies: prove the sensor answers before believing a zero.

**Hardware revision 2** was built and passed bench bring-up on 2026-09-13 (§12.2), passed §15
Phase 2 in the woodshop the same day (§11.7), and logged ten hours at 0.005 disturbers/min in its
enclosures (§11.8). Both boards are soldered per §7.5, joined by a swappable patch cable, on a
2 A USB brick through a 22 AWG cable. The mains supply was built and abandoned — see §5.

**Still broken, still deliberately deferred: the per-strike path into Home Assistant (§8.4).**
The storms confirmed the diagnosis rather than overturning it — roughly 12 Storm Alert state
changes reached Home Assistant against ~880 recorded energy changes, so about **1% of strikes**.
Distance and energy *do* land, because real strikes vary and HA records changes; that is what
made §11.9 readable at all. It is not a per-strike event stream, and it is not enough. The design
for the fix is in [`strike-pipeline-design.md`](strike-pipeline-design.md).

**Still owed:** 5 V at the ESP32 `5V` pin with WiFi active on the 22 AWG cable (§12 step 2); a
liveness check in the attic; deployment tuning (`indoor: false`, and `spike_rejection` off its
bench floor of 1 — §15 Phase 3). Note that the bench-tuned configuration is what detected the
storms, so retuning now changes a demonstrably working instrument: do it as a one-variable
experiment, not as a cleanup.

**Not established:** no individual strike has been cross-checked against lightningmaps.org, and
there are still no per-event timestamps. The WS90 correlation in §11.9 is hourly, and it is the
only independent confirmation the project has.
...
**Measured against real lightning, 2026-09-19 to 09-22 (§11.9): exactly that.** Three storms
produced ~880 energy state changes and 68 distance state changes in Home Assistant — the varied
values did land, as predicted — against roughly **12 Storm Alert state changes, about 1% of
strikes**. So the prediction in the paragraph above is now a measurement, in both halves. The
consequence is unchanged: **no automation can trigger on Storm Alert**, there is still no strike
count and no per-event timestamp, and the fix is still the counter (or better, the MQTT pipeline
in [`strike-pipeline-design.md`](strike-pipeline-design.md)).
...
**Installed 2026-09-22 12:20 in the garage / breakfast / laundry attic** (over the single-story wing) — the location chosen here, now occupied. See §11.10 for the install and its first afternoon.

**The bedroom closet, 2026-09-18 to 2026-09-22, did its job and is released.** It was taken as a waypoint that might become permanent: an exterior corner, little wiring nearby, easy access, and far cooler than a 52 °C attic, which roughly doubles electrolytic life (§9). It met both tests that were set for it — quiet (zero false lightning in four days) *and* it caught real storms (§11.9), which is more than the attic has ever been asked to do. The move to the attic went ahead anyway, as the location chosen from the start; the closet's four days are better read as the validation campaign that finally proved the instrument (§11.9) than as a site trial it lost. **It remains the known-good fallback**, and unlike every earlier candidate it now has a measured baseline behind it.
...
- **Real validation requires a live storm** cross-checked against lightningmaps.org — see below. There is no bench substitute. **Achieved 2026-09-19 to 09-22 (§11.9)**, though by correlation against the WS90 rather than against lightningmaps.org, which still needs per-event timestamps.
...
### 11.9 Real lightning, at last: three storms from the bedroom closet — 2026-09-19 to 09-22

**This is the validation the project had never had.** §11 said flatly that there is no bench
substitute for a live storm and that every `INT_L` recorded up to then was believed false. Three
storms passed over between 2026-09-19 and 2026-09-22 while the node sat in the bedroom closet,
and the node detected all three.

**Configuration and site, unchanged throughout:** both boxes in the bedroom closet since
2026-09-18, bench-tuned (`indoor: true`, `spike_rejection: 1`, `noise_level: 2`,
`watchdog_threshold: 2`, `mask_disturber: false`, `lightning_threshold: 1`), 22 AWG USB cable,
Samsung 2 A brick. Four reboots in the seven-day window: three on 09-18/09-19 during setup, and
the 09-22 12:20 move to the attic (§11.10).

#### How this was read, and what that costs

No survey was running — nobody was at a terminal for any of the three storms. The evidence is the
§15 Phase 4 "passive net": Home Assistant's own record, read out of Prometheus.

- **`hass_state_change_total{entity="sensor.esp32_lightning_sensor_lightning_energy"}`** is the
  workhorse. Energy varies from strike to strike, so nearly every strike is a state change, and a
  *counter* cannot be missed between scrapes the way a gauge can.
- `..._lightning_distance` changes far less often (68 times against 884) because the distance
  *code* repeats across nearby strikes. Its value, sampled, is what gives the distance range.
- `hass_sensor_unit_strikes{entity="sensor.gw3000b_lightning_strikes"}` is the WS90's own daily
  count — an independent AS3935 in the same house, outdoors, and the only cross-check available.

⚠️ **Three limits on everything below.** First, the energy counter is a **lower bound**: two
consecutive strikes carrying the same energy register once. Second, there are **no per-event
timestamps** — counter increases were bucketed by hour, which is the whole resolution of this
dataset. Third, **only `INT_L` reaches Home Assistant**, so this says *nothing* about the closet's
disturber or noise-floor rates; they were never measured there. All three limits are exactly what
the [`strike-pipeline-design.md`](strike-pipeline-design.md) MQTT pipeline exists to remove.

#### The three storms

Hourly, local time. "Node" is the count of energy state changes; "WS90" is that hour's increase in
the weather station's strike count.

| Hour | Node | WS90 | Distance codes seen (km) | Energy range |
|---|---|---|---|---|
| **09-19 16:00** | 1 | 3 | 37 | 1,894 |
| 09-19 17:00 | 11 | 5 | 5, 6, 34 | 3,093 – 211,649 |
| **09-19 18:00** | **208** | **85** | 1, 5, 6 | 1,856 – 256,769 |
| **09-19 19:00** | **295** | 48 | 1, 5, 6 | 2,825 – 218,619 |
| 09-19 20:00 | 91 | 55 | 5, 6, 8 | 2,550 – 167,588 |
| 09-19 21:00 | 5 | 1 | 10, 17, 34 | 2,412 – 20,943 |
| **Storm A total** | **611** | **197** | | |
| 09-20 17:00 | 2 | 0 | 27, 31 | 3,217 – 6,678 |
| **09-20 18:00** | 69 | 10 | 1, 5, 6, 24 | 2,650 – 325,864 |
| 09-20 19:00 | 25 | 0 | 6, 14, 20, 24 | 1,590 – 152,566 |
| 09-20 20:00 | 8 | 3 | 6 | 5,482 – 159,982 |
| **Storm B total** | **104** | **13** | | |
| 09-21 15:00–19:00 | 8 | 5 | 12, 17, 27, 31 | 1,502 – 40,601 |
| **Distant activity** | **8** | **5** | | |
| 09-22 04:00 | 10 | 10 | 8, 12, 31, 34 | 2,807 – 88,055 |
| 09-22 05:00 | 1 | 1 | 31 | 3,384 |
| 09-22 06:00 | 12 | 6 | 8, 10, 12, 14, 24, 31 | 3,075 – 62,788 |
| **09-22 07:00** | 71 | 29 | 5, 6, 8, 10 | 3,601 – 254,338 |
| 09-22 08:00 | 34 | 14 | 1, 6, 8 | 4,595 – 323,655 |
| 09-22 09:00 | 23 | 13 | 12 | 5,268 – 170,783 |
| 09-22 10:00 | 1 | 2 | 27 | 6,848 |
| **Storm C total** | **152** | **75** | | |

**Outside those four windows, in seven days, the node produced nothing.** Every other state
change in the series is accounted for by the four reboots (two per entity, as the entity goes
unavailable and back) plus one availability gap at the start of the range. That is the number
that matters as much as the detections: **zero false lightning across four days in the closet**,
against the breadboard's 3–8 per minute (§11.2).

#### What makes these real, and not more §11.2

Four independent things, and the case rests on all four together rather than any one:

1. **The distances are distances.** Fourteen distinct codes appeared — **1, 5, 6, 8, 10, 12, 14,
   17, 20, 24, 27, 31, 34, 37 km** — and they track the storms sensibly: distant codes (27–37 km)
   as activity began on 09-19 at 16:00 and again on 09-21, collapsing to 5–6 km through the
   09-19 18:00–20:00 peak, then back out to 10–34 km as it left at 21:00. The bench's false
   lightning was **1.0 km, every single time** (§11.2).
2. **`63` never appeared.** Not once in seven days. That is the out-of-range code §8.2 warns
   reads as a plausible 63 km strike, and it was the value that made the 34.7-hour August dataset
   worthless. Its complete absence is the single cleanest signal here.
3. **The energies vary over two orders of magnitude**, ~1,500 to ~326,000, and the largest land
   in the peak hours. §11.3 found roughly a quarter of false events carried non-zero energy, so
   energy alone was never a discriminator — but this distribution does not resemble that one.
4. **An independent detector in the same house saw the same storms in the same hours.** The WS90
   is not a strong cross-check (it is another AS3935, it aggregates, and it is the very sensor
   whose data exposure started this project) but it is a separate instrument with separate
   optics, and its three busy periods are the node's three busy periods.

#### What this does not show

- **The node counts about three times what the WS90 counts** in storm A (611 against 197) and
  twice in storm C. Several readings fit and none is established: the AS3935 at
  `lightning_threshold: 1` reports every event it classifies including intra-cloud activity, the
  WS90 aggregates and reports on its own schedule, and storm-correlated EMI — rain static, nearby
  strikes coupling into house wiring — would also land in exactly these hours. **A per-strike
  count from this node is not yet a trustworthy number; the presence and timing of storms is.**
- **The WS90 reports 1–4 strikes an hour, around the clock, on clear nights**, which the node
  never corroborates. Either the WS90 has a false-positive floor the node does not, or an
  outdoor sensor hears distant strikes an indoor one misses. Both are plausible; this dataset
  cannot separate them, and it is worth remembering before treating the WS90 as ground truth.
- **No individual strike was cross-checked against lightningmaps.org.** §11 asks for that and it
  has still not been done — hourly bucketing is too coarse for it to mean much anyway. It needs
  per-event timestamps, which needs the §8.4 fix.
- **Nothing about disturbers or the noise floor**, in the closet or anywhere else, follows from
  this data. Those classes never leave the node.

#### The per-strike path, measured against real strikes

§8.4 predicted that Storm Alert would not survive the trip to Home Assistant, and real lightning
confirms it: **about 12 Storm Alert state changes reached HA against ~880 energy changes**, or
roughly 1%. What did work is the thing §8.4 called benign — distance and energy advanced, because
real strikes vary and Home Assistant records changes. That is the only reason this section exists.

**This is a reprieve, not a fix.** The passive net caught *that* storms happened and roughly how
close. It cannot produce a strike count, a per-strike timestamp, or an automation trigger, and
two identical consecutive strikes are still one data point. Everything §15 Phase 4 and
[`strike-pipeline-design.md`](strike-pipeline-design.md) say remains true.

### 11.10 Installed in the garage attic — 2026-09-22

**Both boxes moved together from the bedroom closet to the garage attic at 12:20**, the location
§10 chose a year of reasoning ago and the first install intended to be permanent. The node
rebooted at 12:20 (uptime read 61.9 s at 12:21) and has run unbroken since. Nothing else changed:
same brick, same cable, same patch cable, same bench-tuned configuration.

First afternoon, read from the node's own diagnostics:

| | Woodshop (§11.8) | Bedroom closet | Garage attic |
|---|---|---|---|
| WiFi | −62 dBm | −31 to −40 dBm | **−49 to −55 dBm** |
| ESP32 internal temperature | — | 38–44 °C | **37.8 °C at 12:21 → 50 °C by 15:20**, holding 50.0–50.6 °C to 17:11 |
| Lightning classifications | 0 in ~14 h | 0 outside storms in 4 days | **0 in the first 5 h** |

- **WiFi is comfortable.** §15 Phase 3 flagged signal as the risk at an exterior location — a node
  that keeps dropping leaves gaps that read as quiet. −49 to −55 dBm is worse than the closet and
  better than the woodshop, with no reconnects in five hours.
- **The thermal figure lands where §10 said it would.** The die reads above ambient, and ~50 °C
  on a September afternoon is consistent with the attic's ~52 °C summer peak from two years of
  data. The 105 °C electrolytics (§9) are the right call and the vented main box is doing its job.
  Worth re-reading next July, which is the real test.
- ⚠️ **The five quiet hours are not yet evidence.** §11.7's rule: a zero from a quiet spot means
  nothing until the sensor has been proved alive there, and a patch cable disturbed in the move
  produces exactly this output. **The emulator liveness check is the next thing to do**, before
  any attic number is quoted.

**Still to do at this site,** in order: the liveness check; a multi-day survey through
`tools/ws-log-bridge.py` to get the disturber and noise-floor rates the Home Assistant path
cannot show (§11.9); the owed 5 V reading (§12 step 2); and only then deployment tuning
(`indoor: false`) as a one-variable change against the configuration that demonstrably works.
...
- **The out-of-range `63` code is the tell, and its absence is the result.** Every pre-rebuild "strike" reported `63 km` or `1.0 km`; across three real storms the node reported fourteen distinct codes from 1 to 37 km and `63` not once (§11.9). When a sensor's distance output is a lookup table, the *distribution of codes* is a better authenticity test than any single reading.
- **A broken event path can still prove a sensor works.** §8.4 has blocked the per-strike deliverable since July, and the storms were still caught — not by the pulse that was designed to carry them, but by the two entities whose *values change* when the input is real. Varied data leaks through a state-based store in a way monotonous data cannot. It is a reprieve, not a fix (§11.9).
- **Validation arrived by waiting, not by building.** The instrument was finished on 2026-09-18 and the question "does it detect lightning?" was answered four days later by weather, with no further work. §11.1 said there is no bench substitute for a live storm; the corollary is that once the instrument is trustworthy, the cheapest next step may be to leave it alone and watch.
- **An independent sensor is worth having even when it is the same chip.** The WS90 is another AS3935 and is the very sensor whose data exposure started this project, and it was still the only corroboration available for §11.9. It also disagreed usefully: it reports 1–4 strikes an hour around the clock on clear nights, which the node never echoes.

## 14. Deliverables

**Repository layout, reorganised 2026-09-22.** The project directory now carries only the finished
article; this notebook and the other historical and planning documents moved into `docs/`.

```
README.md                     the finished project: overview, BoM, build steps, photos
lightning-detector.yaml       complete ESPHome configuration
as3935-node-wiring.pdf        printable point-to-point wiring set  (make-wiring-diagram.py)
as3935-protoboard-layout.pdf  hole-by-hole board layouts           (make-protoboard-layout.py)
DRAWING-VERSION               shared date stamp printed in both PDFs' footers
images/                       build photographs, with 640 px thumbnails in images/thumbs/
tools/                        measurement instruments
sen39002-emulator-uno/        PlatformIO project for the emulator shield
docs/
  project-notebook.md         this file — the full engineering history
  strike-pipeline-design.md   the MQTT per-strike pipeline, designed, not built
  sdr-interference-hunting.md the noise-floor investigation, if it is ever needed
```
...
- `docs/strike-pipeline-design.md` — the per-strike MQTT pipeline: why the Home Assistant path cannot carry an event stream (§8.4, and §11.9 measured it at ~1% of strikes), the event contract, the data rules, and the build order. **Design only; nothing built.**
- `images/` — build photographs from 2026-09-15, 2048 px, with 640 px thumbnails in `images/thumbs/` for the README.
- `docs/project-notebook.md` — this document. Every `§` reference in the repository points into it.
...
- **The bedroom closet is done (2026-09-18 to 09-22).** Four days, zero false lightning, and three real storms detected — §11.9. It never got a disturber or noise-floor number, because those classes do not reach Home Assistant and no survey was running; that is the one thing the closet leg is missing.
- **Baseline the garage attic (current step, from 2026-09-22 12:20).** Installed — §11.10. WiFi and thermals are read and fine; nothing else is. In order: **(1) the emulator liveness check**, which is blocking, because five quiet hours and a patch cable knocked loose in the move are the same observation (§11.7); **(2) a multi-day survey** through `tools/ws-log-bridge.py` into `tools/ambient-survey.py`, keeping the raw stream, long enough to see several daily heat cycles — this is the only way to get the attic's disturber and noise-floor rates, which §11.3's 0.6/min average never credibly gave. The closet is the fallback if it reads badly.
...
- **Tune for deployment — carefully, and last.** `indoor: false` for attic AFE gain, and `spike_rejection` back off its bench floor of 1. ⚠️ **This is no longer a cleanup.** The bench-tuned configuration is the one that detected three real storms at 5–37 km with zero false lightning (§11.9), so every one of these knobs now has a working reference to be measured against. Change one at a time, through a storm season if that is what it takes, and read `INT_L` *alongside* the disturber rate when judging, not instead of it (§11.3).

### Phase 4 — Make it actually deliver

- **Catch a real storm. ✅ Done, 2026-09-19 to 09-22 — §11.9.** Three storms, distance codes across
  **5–37 km**, energies spanning two orders of magnitude, no `63` at all, correlating hour for hour
  with the WS90. The passive net below is exactly what caught them, so it is kept as written.
  **What the storms did *not* settle**, and what a future one still should: a per-strike count that
  can be believed (the node logged ~3× the WS90's count — §11.9), and a timestamp-level cross-check
  against lightningmaps.org, which needs the per-strike path fixed before it is worth attempting.
